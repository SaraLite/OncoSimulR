...
**Attention**
The payoff values are normalised in the range (0:1) so 1 represents the maximum fitness for any given phenotype.
...
* $\rho\varrho$ (p): beneficio de D por factores de crecimiento paracrinos producidos por I.
...
```{r}
# No entiendo pq hay que meter un WT, para mí ya sería S == WT
# Tp entiendo pq 1 + en todos
...
Situación extrema: a pesar de que el coste de la independencia es bastante bajo (solo 0.3 en relación a escenarios anteriores), el hecho de que el ambiente sea moderadamente rico (b = 0.35), hace que las células D (dependientes del microambiente) prevalezcan sobre el resto, incluso sobre las I. 
Con hacer solo un poco más pobre el microambiente (b = 0.40), las células D y las I sobreviven, cooperando entre sí (Cuarto escenario).
...
### Simulación con 100 réplicas delescenario 1
...
# sampledGenotypes(samplePop(simulScen1))
# simulation <- tail(simulation[[1]], 1)[1, -1]
```
