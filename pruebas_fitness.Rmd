...
The parameters considered in Lotka-Volterra model are:
  ...
  a_ij -> competition coefficient. The effect of species j on species i

With these parameters, the continuous-time differential equations for the Lotka-Volterra model are:

![Lotka-Volterra continuous-time differential equations](.\Images_markdown\lotka_volterra_continuous_equations.PNG)

As it is said above, the signs of α12 and α21 therefore reflect the relationship between the two species:
...
We must introduce a WT species in the model due to "to_genotFitness_std" which requires WT to appears once.
...
S2 in these examples has a higher rate of growth than S1 (r2 > r1) and higher carrying capacity (K2 > K1). However, S1 has a stronger competitive effect on S2 (a21 > a12).
...
No hay competitividad, los coeficientes de competitividad = 0
...
S1S2_competition <-
  allFitnessEffects(
    genotFitness =
      S1S2_LV(1.2, 1.7, 350, 400, 0, 0,
              gt = c("WT", "S1", "S2")),
    frequencyDependentFitness = TRUE,
    frequencyType = "abs",
    spPopSizes = c(5000, 5000, 5000)) ## frequency expressed in absolute terms



## Plot fitness landscape of genotype "H, D" evaluation
data.frame("S1_fitness" = evalGenotype(genotype = "S1", fitnessEffects = S1S2_competition), 
           "S2_fitness" = evalGenotype(genotype = "S2", fitnessEffects = S1S2_competition))
...
S1S2_competition <-
  allFitnessEffects(
    genotFitness =
      S1S2_LV(1.2, 1.7, 350, 400, 0.25, 0.5,
              gt = c("WT", "S1", "S2")),
    frequencyDependentFitness = TRUE,
    frequencyType = "abs")

S1S2_simulation <- oncoSimulPop(2, 
                                S1S2_competition,
                              model = "Exp",
                              onlyCancer = FALSE,
                              finalTime = 500,
...
S1S2_competition <-
  allFitnessEffects(
    genotFitness =
      S1S2_LV(1.2, 1.7, 350, 400, 0.77 , 1,
              gt = c("WT", "S1", "S2")),
    frequencyDependentFitness = TRUE,
    frequencyType = "abs")
...
S1S2_competition <-
  allFitnessEffects(
    genotFitness =
      S1S2_LV(1.2, 1.7, 350, 400, 0.9, 1.4,
              gt = c("WT", "S1", "S2")),
    frequencyDependentFitness = TRUE,
    frequencyType = "abs")
